...
async function getRelevantImage(productName: string): Promise<string | null> {
  ...
  if (productName.includes('Дрель')) {
    if (productName.includes('Ударная')) {
      query = 'hammer drill impact driver power tool';
      ...
      query = 'drill bit set screwdriver bits accessories';
    } else {
    ...
    if (productName.includes('Потолочный')) {
      query = 'ceiling led light fixture lamp modern';
    } else if (productName.includes('Настенный')) {
      query = 'wall led light sconce fixture';
    } else if (productName.includes('RGB')) {
      query = 'rgb led strip light colorful smart';
    } else if (productName.includes('Умный') || productName.includes('SmartHome')) {
      query = 'smart led bulb light wifi control';
    } else if (productName.includes('Пульт ДУ')) {
      query = 'led bulb light remote control';
    } else {
      query = 'led light bulb lamp bright';
    }
  } else {
...
async function fixSpecificPositions() {
  ...
  const positionsToFix = [1,7,9,11,13,15,17,19,20,21,23,24,25,28,29,33,34,36,37,39,41,45];
...
fixSpecificPositions().catch(console.error);
